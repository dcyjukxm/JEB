...
public class cd extends eI
{
    ...
    public static void dN(final String s, final String s2, final String s3, final String[] array) {
        new cd(s, s2, s3, array).rn();
        System.exit(0);
    }
    
    public cd(final String s, final String s2, final String s3, final String[] array) {
        super(s, s2, s3, array);
    }
    ...
    public void rn() {
        ...
        if (this.rK.ld(WV.bG)) {
            if (!uK.ld()) {
                cn.dN(String.format("(Update checks can be disabled by setting '%s' to 'false' in the configuration file.)", WV.bG.ld()));
            }
            this.LR();
        }
        if (this.cb != null) {
        ...
        if (this.PF != null) {
            final QA dn = this.dN(this.PF, true, null);
            if (dn == null) {
                cn.dN("An error occurred, the script could not be loaded");
            }
            else if (dn.dN() != QA.uK.QE) {
    ...
    @Override
    public boolean dN(final String cb) {
        if (this.rK()) {
            return false;
        }
        cn.dN(String.format("Opening %s", cb));
        if ((TM.KK(cb) & 0xFFFFFF) == 0x42444A) {
            this.Ux = this.wU(cb);
        }
        else {
            this.Ux = this.dN(cb, false);
        }
        if (this.Ux == null) {
            cn.dN("Invalid file or file does not exist");
            return false;
        }
        (this.ZY = new AR(this.Ux.ld("classes.dex"))).dN(this.rK.ld(WV.kq));
    ...
    public boolean ld(final String s) {
        ...
        return line != null && line.trim().equalsIgnoreCase(rP.dN(new byte[] { 20, 28, 22 }, 1, 109));
    }
    
    @Override
    public void ci() {
        System.out.println(String.format("This is your first run of JEB v%s. The changelist can be found at %s.", cd.dN.toString(), "http://www.android-decompiler.com/changelist.php"));
    }
    ...
    public String QE(final String s) {
        System.out.println(String.format(rP.dN(new byte[] { 11, 10, 28, 21, 29, 73, 66, 27, 90, 0, 113, 12, 92, 0, 95, 92, 92, 87, 12, 65, 18, 92, 88, 80, 73, 78, 60, 23, 65, 8, 9, 28, 0, 21, 1, 68, 90, 28, 23, 0, 35, 49, 49, 14, 65, 50, 14, 0, 31, 73, 14, 26, 0, 94, 90, 73, 44, 9, 76, 79, 2, 12, 21, 9, 0, 26, 79, 28, 69, 4, 12, 30, 26, 69, 11, 64, 47, 22, 80, 13, 19, 2, 2, 72, 21, 0, 78, 6, 94, 0, 66, 92, 90, 92, 66, 68, 65, 30 }, 2, 212), "              "));
        System.out.println(String.format(rP.dN(new byte[] { 62, 60, 9, 4, 18, 22, 69, 86, 31, 26, 26, 29, 84, 5, 86, 95, 12, 65, 15, 10, 68, 85, 6, 22, 69, 84, 28, 13, 69, 70, 9, 3, 0, 3, 24, 30, 7, 9, 71, 2, 78, 5, 10, 6, 11, 29, 22, 69, 68, 5, 21, 21, 67, 2, 66, 14, 3, 13, 66, 84, 27, 79, 71, 2, 11, 11, 23, 19, 21, 17, 69, 65, 65, 75, 14, 28 }, 1, 110), "http://www.android-decompiler.com/genlk.php"));
        System.out.println(String.format(rP.dN(new byte[] { 15, 6, 19, 28, 28, 26, 2, 72, 16, 65, 92, 2, 19, 0, 20, 74 }, 2, 246), s));
